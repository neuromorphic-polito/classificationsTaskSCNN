...
def main(datasetName, encoding, filterbank, channels, bins, structure, quartile):
    ...
    modelCNN = tf.keras.models.load_model(f'{sourceFolder}{datasetName}{filterbank}{channels}x{bins}{structure}{quartile}{encoding}.keras', custom_objects={'Relu': Relu, 'Masking': Masking})
...
if __name__ == '__main__':
    ...
    fileName = f'{sourceFolder}{datasetName}SCNN-ModelPruned.csv'
    try:
    ...
    if flagCompute == True:
        accuracyTrain, accuracyTest, synapses = main(datasetName, encoding, filterbank, channels, bins, structure, quartile)
